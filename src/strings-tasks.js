...
function countVowels(str) {
  if (!str || !str.match(/[aeiouyAEIOUY]/g)) return 0;
...
function findLongestWord(sentence) {
  ...
  let logestWord = arr[0];
  for (let i = 1; i < arr.length; i += 1) {
    if (logestWord.length < arr[i].length) logestWord = arr[i];
  }
  return logestWord;
}
...
function encodeToRot13(str) {
  const inpStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rotStr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (inpStr.indexOf(str[i]) !== -1) result += rotStr[inpStr.indexOf(str[i])];
    else result += str[i];
  }
  return result;
}
